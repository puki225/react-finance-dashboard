...
//
// blendedPrice1 is ONE reference — the whole filtered selection's own blended price,
// not recomputed per group — so a member's mix can only ever reflect where IT sits
// relative to the overall blend, not some drill-down-relative story. (An alternative,
// group-relative version was tried and reverted: it made mix react to how the table
// happens to be grouped, which was more confusing than useful in practice.)
//
...
async function pvmBaseRows(dateFrom, dateTo, channel) {
  ...
  const shopifySql = `
    ...
    shopify_refunds AS (
      SELECT
        ...
        -- refund AMOUNT per transaction) — left at 0 rather than guessing, so net units
        -- below is exact for Amazon and simply un-netted (matches today's behaviour)
        -- for Shopify, instead of quietly feeding a fabricated count into price math.
  ...
  const result = await pool.query(`
    ...
    SELECT
      ...
      sp.image_url,
      -- NET of returned units (Amazon: exact, from quantity_refunded; Shopify: not
      -- netted, see shopify_refunds above) — a return is a genuinely lost unit, not
      -- just lost revenue, so PVM's Volume term should move with it instead of the
      -- whole swing landing in Price/Mix as it would off a gross unit count.
  ...
  try {
    ...
    const accumulate = (rows, fx) => {
      ...
      for (const r of rows) {
        ...
        const key = keyOf(r);
        // asin/sku/product_name/image_url are carried through purely for display (the
        // product image/SKU/ASIN cell at ASIN level) — they don't feed the price/volume
